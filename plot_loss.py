...
def plot_loss(csv_files, labels, save_plot=True, show_plot=False, combine=False):
    ...
    for csv_file, label in zip(csv_files, labels):
        ...
        epochs = data['epoch']
        losses = data['loss']
        plt.plot(epochs, losses, label=f'{label} Loss', markersize=3, linewidth=1)
    ...
    if save_plot:
        if combine:
            ...
            save_path = ospj(os.path.commonpath(csv_files), 'combined_loss_plot.png')
            plt.savefig(save_path)
            print(f'Combined plot saved at: {save_path}')
        else:
            # Save individual plots
            for csv_file in csv_files:
                save_path = ospj(os.path.dirname(csv_file), 'loss_plot.png')
                plt.savefig(save_path)
...
def main():
    ...
    if args.combine:
        plot_loss(csv_files, labels, args.save_plot, args.show_plot, combine=True)
    else:
        for csv_file, label in zip(csv_files, labels):
            plot_loss([csv_file], [label], args.save_plot, args.show_plot)
