...
def prepear_data_argparse(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    prepear_data_parser.add_argument('--original_data', type=str, default=ospj(DATA_FOLDER, 'BengaliWords_CroppedVersion_Folds'), help='Path to the data directory')
    prepear_data_parser.add_argument('--fold', type=int, default=0, help='Fold number')
    prepear_data_parser.add_argument('--folds', nargs='+', type=int, default=[0], help='What folds to prepear')

    prepear_data_parser.add_argument('--output_dir', type=str, default=ospj(DATA_FOLDER, 'BengaliWords_CroppedVersion_Folds'), help='Path to the output directory')
    ...
    prepear_data_parser.add_argument('-s', '--split', action='store_true', help='Split the training data to add validation data')
